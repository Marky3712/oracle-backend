...
class GigaChatClient:
    ...
    async def generate_image(self, prompt: str) -> str:
        """Генерирует картинку и возвращает base64"""
        token = await self._get_token()
...
async def generate_post_with_image(topic: str, hour: int) -> dict:
    """
    Генерирует пост и картинку через GigaChat
    hour: 7, 12 или 17 (для разного стиля)
    """
    try:
        if hour == 7:
            style_prompt = "Напиши короткий, вдохновляющий гороскоп на день. 3-4 предложения. Используй эмодзи."
            image_style = "Астрологическая карта, знаки зодиака, мистическое звёздное небо, золотые символы. Готический стиль, тёмные тона, золотые акценты."
        elif hour == 12:
            ...
            image_style = f"Мистическая иллюстрация на тему: {topic}. Готический стиль, тёмные тона, золотые акценты, магическая атмосфера. Без текста."
        else:
            ...
            image_style = f"Атмосферная иллюстрация к легенде или мистической истории на тему: {topic}. Стиль: тёмная фэнтези, готика, уютная магия. Без текста."
        ...
        # Генерация картинки
        image_base64 = await gigachat.generate_image(image_style)
...
async def send_post_to_channel(chat_id: str, bot_token: str, post_text: str, image_base64: str = None):
    """Отправляет пост в канал (текст + картинка, если есть)"""
...
async def midday_post_job():
    ...
    if result["success"]:
        ...
        print(f"Ошибка генерации поста: {result.get('error')}")

async def evening_post_job():
    """Отправляет пост в 17:00"""
...
async def test_image():
    ...
    try:
        image_base64 = await gigachat.generate_image("Мистический кристалл, готический стиль, тёмные тона, золотые акценты")
        return {"success": True, "image_preview": image_base64[:100] + "..."}
    except Exception as e:
        return {"success": False, "error": str(e)}

@app.get("/api/test-midday")
async def test_midday():
    """Тест отправки дневного поста"""
